...
import {
  ...
  FileSpreadsheet, HelpCircle, Sparkles
} from 'lucide-react'
...
const SECTIONS = [
  ...
  },
  {
    ...
    body: () => (
      <>
        ...
        <Sub>手動入力でメニューをまとめて書く</Sub>
        <P>「手動入力」ボタンを押すと、メニューを1行ずつテキストで入力できます。決まった書式で書くと、各種目がカードに展開され、パフォーマンス記録表にも反映されます。</P>
        <Bullets items={[
          '1行＝1種目。形式は「種目名 重量kg 回数」または「種目名 重量kg 秒数s」。',
          '区切りは半角スペース1つ。重量・回数・秒数はすべて半角英数字で入力します。',
          '重量は末尾に kg（例 60kg / 12.5kg）。重量がなければ省略できます。',
          '秒数は末尾に s（例 30s）。回数は数字のみ（例 10）。',
          '例：「ベンチプレス 60kg 10」→ 60kg×10回／「プランク 30s」→ 30秒／「ラットプルダウン 8」→ 8回。'
        ]} />
        <Note type="warn">全角スペースや全角数字が混ざっている行、書式を満たさない行は反映されず、警告として表示されます。半角で入力し直してください。</Note>
        <Sub>回数券の消費</Sub>
  ...
  },
  {
    ...
    body: () => (
      <>
        ...
        <Steps items={[
          '「新規購入」ボタンを押します。',
          '券種を選ぶと、枚数と金額が自動で入ります（8回数券／12回数券／単発利用）。',
        ...
        <Sub>単発利用（¥4,980）</Sub>
        <P>1回だけ利用する方向けの券種です。「新規購入」で「単発利用」を選ぶと1回分（¥4,980）が登録されます。</P>
        <Bullets items={[
          '単発利用のみの会員は、回数券の残数アラート（残2回以下）の対象外になります。',
          '最終利用日から6ヶ月で退会予定として扱い、ダッシュボードに退会アラートが出ます（→「ダッシュボードのアラート対応」章）。',
          '退会予定日の5日前から「退会予定日が近づいています」、予定日以降は「退会処理が完了していません」と表示されます。',
          '会費ペイでコース削除を済ませたら「コース削除実施済み」を押すとアラートが消えます。再び単発利用を購入すると、退会アラートは最新の利用日から数え直します。'
        ]} />
      </>
    )
  },
  ...
  },
  {
    ...
    body: () => (
      <>
        ...
        <Sub>月額プラン休会手続きの期間設定</Sub>
        <P>「月額プラン休会手続き」を選ぶと、休会終了日を「◯月末」（1〜12月）から選べます。休会開始日は受付日から自動で決まります。</P>
        <Bullets items={[
          '休会開始日（自動）：受付が毎月10日まで → 翌月1日／11日以降 → 翌々月1日。',
          '休会終了日：選んだ「◯月末」を、開始日以降で最も近い年に当てはめて自動計算します。',
          '受付フォームと受付履歴に、計算された休会開始日・終了日が表示されます。'
        ]} />
  ...
  },
  {
    ...
    body: () => (
      <>
        ...
        <Sub>「アラート」タブと「ダッシュボード」タブ</Sub>
        <P>ダッシュボード上部のタブで表示を切り替えられます。「アラート」タブは現在対応が必要なアラートをすべて1か所にまとめて表示します（タブに件数バッジが付きます）。「ダッシュボード」タブは来店数グラフや本日の来店などの集計を表示します。</P>
        <Bullets items={[
        ...
        <Bullets items={[
          ...
          '単発利用の退会対応：最終利用から6ヶ月で退会予定の会員を表示します。退会予定日が近い会員と、予定日を過ぎても退会処理が未完了の会員を分けて表示し、「コース削除実施済み」で消します。'
        ]} />
  ...
  },
  {
    ...
    body: () => (
      <>
        ...
        <P>並び順は自由に変更できます。各行の上下ボタンで手動入れ替えのほか、「部位別」「五十音順」ボタンで一括並び替えができます（変更は自動保存され、ドロップダウンの表示順に反映されます）。</P>
        <Sub>データ管理</Sub>
        <Bullets items={[
  ...
  {
    id: 'updates', title: '更新内容（アップデート履歴）', icon: Sparkles,
    keywords: '更新 アップデート 履歴 新機能 変更点 単発利用 並び替え 手動入力 休会 アラートタブ',
    body: () => (
      <>
        <P>アプリのアップデートで追加・変更された機能をまとめています。新しい機能から順に記載します。</P>
        <Sub>ダッシュボードに「アラート」タブを追加</Sub>
        <P>ダッシュボード上部でタブを切り替えられるようになりました。「アラート」タブに、対応が必要なアラートをすべてまとめて表示します（→「ダッシュボードのアラート対応」章）。</P>
        <Sub>単発利用（¥4,980）に対応</Sub>
        <P>回数券タブの「新規購入」に単発利用を追加しました。残数アラートの対象外とし、最終利用から6ヶ月で退会アラートを表示します（→「回数券・月額プラン」章）。</P>
        <Sub>セッションメニューの手動入力フォーマット</Sub>
        <P>決まった書式（種目名 重量kg 回数／秒数s、半角スペース区切り）で書くと、メニューがカードに展開され、パフォーマンス記録表にも反映されるようになりました（→「セッション記録」章）。</P>
        <Sub>種目プリセットの並び替え</Sub>
        <P>設定の種目プリセットを、手動の上下移動・部位別・五十音順で並び替えられるようになりました（→「設定」章）。</P>
        <Sub>月額プラン休会手続きの期間設定</Sub>
        <P>休会手続きで休会終了日（◯月末）を選べるようになり、休会開始日が受付日から自動で決まるようになりました（→「手続き」章）。</P>
      </>
    )
  },
  {
