...
let lastWriteAt = 0              // 直近の書き込み時刻（操作中かどうかの判定に使う）

const SYNC_CONFIG_FILE = 'sync-config.json'
// db.sync() はネットワーク往復で同期的にメインプロセスをブロックする。
// 保存（=ローカル書き込み）は即完了させ、クラウド同期は操作が落ち着いてから裏でまとめて行う。
const PULL_INTERVAL_MS = 8000    // 他端末の変更を取り込む間隔
const PUSH_DEBOUNCE_MS = 1200    // 書き込み後に同期するまでの猶予。連続保存（一括保存など）を1回にまとめる
const ACTIVE_QUIET_MS = 5000     // 直近の書き込みからこの時間は「操作中」とみなしプル（取り込み）を見送る
...
// 連続保存中は毎回タイマーが延長されるため、操作が止まるまでネットワーク同期は走らない
// → 保存操作そのものが同期で待たされない。
...
// ・push（デバウンス待ち）がある間
// ・同期実行中
// ・直近 ACTIVE_QUIET_MS 以内に書き込みがある（＝操作中）
// これらの間はプルを見送り、保存操作と db.sync() の衝突を避ける。
function startPullTimer() {
  if (syncTimer) clearInterval(syncTimer)
  syncTimer = setInterval(() => {
    if (syncDebounce || syncing) return
    if (Date.now() - lastWriteAt < ACTIVE_QUIET_MS) return
    safeSync()
  }, PULL_INTERVAL_MS)
}

// 保留中の同期を即実行する（アプリ終了時など、確実にクラウドへ反映したい場面用）。
function flushSync() {
...
module.exports = {
  ...
  getSyncStatus, writeSyncConfig, syncNow, flushSync
}
